    ...
    def turn(self, world):
        ...
        if received_spell is not None:
            if received_spell.type == SpellType.HP:
                ...
                #TODO: It will be better if we can count the number of enemies in 8 tiles from it for each unit
                #Then we can add it to variable unit_score
                best_score = 0
                best_unit = my_units[0]
                for unit in my_units:
                    unit_score = unit.hp // 2 + unit.attack + unit.range * 100 * int(unit.target != None) + 1000 * int(unit.target_if_king != None)
                    if(unit_score > best_score):
